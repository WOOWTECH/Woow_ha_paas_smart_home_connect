...
class TunnelStatusData:
    ...
    is_connected: bool
    status: str  # connected, disconnected, error, starting, unknown
...
class TunnelCoordinator(DataUpdateCoordinator[TunnelStatusData]):
    ...
    async def _async_update_data(self) -> TunnelStatusData:
        ...
        # Attempt remote API call; use None as fallback on any failure
        ...
        is_connected, status = self._merge_status(local_running, remote_status)
    ...
    def _merge_status(
        ...
        """Merge local process state and remote API status.

        Returns (is_connected, status) tuple.

        | local    | remote       | -> is_connected | -> status      |
        |----------|--------------|-----------------|----------------|
        ...
        | running  | API fail     | True            | unknown        |
        | stopped  | API fail     | False           | disconnected   |
        """
        ...
        if local_running and remote_status == "connected":
            return True, "connected"

        if local_running and remote_status != "connected":
            return False, "error"

        if not local_running and remote_status == "connected":
            return False, "error"
